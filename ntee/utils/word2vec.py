...
from ntee.utils.wiki_extractor import WikiExtractor
from ntee.utils.wiki_dump_reader import WikiDumpReader
            ...
            for paragraphs in pool.imap_unordered(
                ...
                for paragraph in paragraphs:
                    ...
                    if learn_entity:
                        ...
                        words = para_words
                    f.write(str.encode(" ".join(str(item) for item in words) + '\n'))

        if abstract_db is not None:
            click.echo('Processing paragraphs in Abstract DB...')
            tokenizer = RegexpTokenizer()

            for key, value in abstract_db.items():
